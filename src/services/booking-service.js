...
async function cancelBooking(bookingId){
    const transaction=await db.sequelize.transaction();
    try {
        const bookingDetals=await bookingRepository.get(bookingId,transaction);
        if(bookingDetals.status == CANCELLED){ 
            await transaction.commit();
            return true;
        }

        await axios.patch(`${ServerConfig.FLIGHT_SERVICE}/api/v1/flights/${bookingDetals.flightId}/seats`,{seats:bookingDetals.noOfSeats,dec:0}); 
...
async function cancelOldBooking(){
    try {
        const time=new Date(Date.now()-1000*300); // for 5 min ago
        const response=await bookingRepository.cancelOldBooking(time);
        console.log(response);
        return response;
    } catch (error) {
        console.log("Error in Cancel Old Booking"+error);
    }
}
